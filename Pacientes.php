...
if (isset($_GET['nombre'])) {
  $colname_qPacientes = $_GET['nombre'];
}
mysql_select_db($database_cnConsultorio, $cnConsultorio);
$query_qPacientes = sprintf("SELECT matricula, fechaderegistro, nombre, edad, sexo FROM pacientes WHERE nombre LIKE %s", GetSQLValueString("%" . $colname_qPacientes . "%", "text"));
$qPacientes = mysql_query($query_qPacientes, $cnConsultorio) or die(mysql_error());
$row_qPacientes = mysql_fetch_assoc($qPacientes);
$totalRows_qPacientes = mysql_num_rows($qPacientes);$colname_qPacientes = "-1";
if (isset($_GET['nombre'])) {
  $colname_qPacientes = $_GET['nombre'];
}
mysql_select_db($database_cnConsultorio, $cnConsultorio);
$query_qPacientes = sprintf("SELECT matricula, nombre, edad, sexo, lugardeorigen FROM pacientes WHERE nombre LIKE %s", GetSQLValueString("%" . $colname_qPacientes . "%", "text"));
$qPacientes = mysql_query($query_qPacientes, $cnConsultorio) or die(mysql_error());
$row_qPacientes = mysql_fetch_assoc($qPacientes);
...
<html>
<head>
<title>Consultorio ULV : Pacientes</title>
...
<!----->
		        ...
		        <ul class=" nav_1">
					...
					<li class="dropdown">
		              ...
		              <ul class="dropdown-menu " role="menu">
		                <li><a href="login.php" ><i class="fa fa-sign-out"></i>Cerrar Sesión</a></li>
       ...
       <div class="content-main">
        ...
        <h3 id="forms-example" class="">Pacientes</h3>
          <table border="1" cellpadding="1" cellspacing="1" class="table table-bordered">
            ...
            <!-- Se hace una consulta mediante una busqueda pasando como parametro el nombre del paciente -->
            <?php do { ?>
              <tr>
                <td><?php echo $row_qPacientes['matricula']; ?></td>
                <td><?php echo $row_qPacientes['lugardeorigen']; ?></td>
                <td><?php echo $row_qPacientes['nombre']; ?></td>
                <td><?php echo $row_qPacientes['edad']; ?></td>
                <td><?php echo $row_qPacientes['sexo']; ?></td>
                <td><a href="DetallesPaciente.php?matricula=<?php echo $row_qPacientes['matricula']; ?>"><i class="fa fa-exclamation-circle" title="Detalles"></i></a></td>
                <td><a href="EditarPaciente.php?m=<?php echo $row_qPacientes['matricula']; ?>"><i class="fa fa-edit" title="Editar Paciente"></i></a></td>
                <td><a href="EliminarPaciente.php?matricula=<?php echo $row_qPacientes['matricula']; ?>"><i class="fa fa-trash" title="Borrar Paciente"></i></a></td>
              </tr>
              <?php } while ($row_qPacientes = mysql_fetch_assoc($qPacientes)); ?>
          </table>
          </div>
          </div>
<div class="clearfix"> </div>
